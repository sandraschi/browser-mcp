...
async def _bruteforce_read_operation(
    ...
    try:
        ...
        if operation == "list_bookmarks":
            ...
            bookmarks = [
                {"id": row[0], "title": row[1] or "", "url": row[2] or "", "dateAdded": row[3], "lastModified": row[4]}
            ...
            cursor.execute(
                "SELECT b.id, b.title, p.url, b.dateAdded, b.lastModified, b.parent FROM moz_bookmarks b LEFT JOIN moz_places p ON b.fk = p.id WHERE b.id = ?",
                (bookmark_id,),
            )
            row = cursor.fetchone()
            if not row:
            ...
            cursor.execute(
                "SELECT b.id, b.title, p.url FROM moz_bookmarks b LEFT JOIN moz_places p ON b.fk = p.id WHERE b.type = 1 AND (b.title LIKE ? OR p.url LIKE ?) LIMIT 100",
            ...
            return {
                ...
                "access_method": method,
                "stats": {"bookmark_count": bookmark_count, "url_count": url_count, "folder_count": folder_count},
            }
        else:
            return {
                "success": False,
...
async def firefox_bookmarks(
    ...
    if operation == "list_bookmarks":
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"Bookmark {bookmark_id} retrieved",
                "bookmark": bookmark,
        ...
        try:
            result = await find_old_bookmarks(
                age_days, str(get_profile_directory(profile_name)) if profile_name else None
            )
            return {
                "success": True,
        ...
        try:
            result = await find_forgotten_bookmarks(
                age_days, str(get_profile_directory(profile_name)) if profile_name else None
            )
            return {
                "success": True,
        ...
        return {
            "success": True,
            "message": "Bookmark refresh (stub)",
            "note": "Full link refresh requires async batch processing",
        }
    elif operation == "get_bookmark_stats":
        try:
