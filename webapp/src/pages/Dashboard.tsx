...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const r = await fetch('http://127.0.0.1:10780/health', { signal: AbortSignal.timeout(5000) });
        if (!cancelled) {
  ...
  return (
    <div data-testid="dashboard" className="space-y-6">
      <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-xl p-6">
        <h1 className="text-2xl font-bold text-zinc-100">Browser MCP</h1>
        <p className="text-sm text-zinc-300 mt-2">Automate Chrome, Firefox, Edge, and Brave over MCP: browse pages, click elements, take screenshots, and manage bookmarks across all major browsers.</p>
        <div className="flex flex-wrap gap-2 mt-4">
          <button type="button" onClick={() => window.open('/bookmarks', '_self')} className="px-3 py-1.5 rounded-lg bg-amber text-zinc-900 text-sm font-medium hover:bg-amber/90">Manage bookmarks</button>
          <button type="button" onClick={() => window.open('/tools', '_self')} className="px-3 py-1.5 rounded-lg bg-zinc-800 text-zinc-200 text-sm font-medium border border-zinc-700 hover:bg-zinc-700">Browse tools</button>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-4" data-testid="kpi-server">
          <p className="text-sm text-zinc-500 mb-1">Server</p>
          <p className="text-lg font-semibold">{health?.service as string || health?.server as string || '...'}</p>
          <p className="text-sm text-zinc-500 mt-0.5">v{health?.version as string || '?'} · port {health?.port as string || health?.backend_port as string || '?'}</p>
        </div>
        <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-4" data-testid="kpi-tools">
          <p className="text-sm text-zinc-500 mb-1">Tools</p>
          <p className="text-lg font-semibold">{health?.tool_count as number ?? tools.length}</p>
          <p className="text-sm text-zinc-500 mt-0.5">registered</p>
        </div>
      ...
      {tools.length > 0 && (
        <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
            {tools.map((t) => (
              <div key={t.name} className="text-sm text-zinc-400 bg-zinc-800 px-3 py-2 rounded border border-zinc-700 truncate">{t.name}</div>
            ))}
          </div>
        </div>
      )}
