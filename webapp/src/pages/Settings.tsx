    ...
    <div data-testid="settings-page" className="space-y-6 max-w-3xl">
      ...
      <section className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-4">
        ...
        <div className="flex items-center gap-2 mt-3">
          ...
          <span className="text-sm text-zinc-400 ml-4">{health?.tool_count ?? '?'} tools registered</span>
        </div>
      </section>
          ...
          <>
            <div className="space-y-2 mb-4">
              {providers.map((p) => (
                <div key={p.name} className="flex items-center gap-2 text-sm">
                  <span
                    className={`w-2 h-2 rounded-full ${p.status === 'detected' ? 'bg-green-500' : 'bg-zinc-600'}`}
                  />
                  <span className="text-zinc-200 capitalize w-24">{p.name}</span>
                  <span className="text-zinc-400">:{p.port}</span>
            ...
            {detected.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm text-zinc-400 mb-1" htmlFor="llm-provider-select">
                    Provider
                  </label>
                  <select
                ...
                <div>
                  <label className="block text-sm text-zinc-400 mb-1" htmlFor="llm-model-select">
                    Model
                  </label>
                  <select
                    id="llm-model-select"
