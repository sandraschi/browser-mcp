    ...
    <div>
      ...
      {!error && skills !== null && skills.length === 0 && (
        <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-6">
          ...
          <p className="text-zinc-500 text-sm mt-2">Skills surface bundled workflows as markdown so agents know how to use the server. This server ships its tool guidance in llms-full.txt instead.</p>
        </div>
      )}
      {skills !== null && skills.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="md:col-span-2 bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-4 whitespace-pre-wrap text-sm text-zinc-300">
            {active ? (active.content || active.description || '(no content)') : <span className="text-zinc-500">Select a skill to view its content.</span>}
          </div>
        </div>
      )}
    </div>
