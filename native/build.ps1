...
# Bundle .env.example (NOT .env - dev .env has personal API keys; leaking it into
# the installer is a CRITICAL security defect per tauri_nsis_building.md)
...
$testPort = 11999
$oldPort = $env:BROWSER_MCP_PORT; $oldHost = $env:BROWSER_MCP_HOST
$env:BROWSER_MCP_PORT = "$testPort"; $env:BROWSER_MCP_HOST = "127.0.0.1"
$testProc = Start-Process -FilePath $src -NoNewWindow -PassThru -RedirectStandardError "$Root\dist\pyi-crash.log"
Start-Sleep -Seconds 5
$env:BROWSER_MCP_PORT = $oldPort; $env:BROWSER_MCP_HOST = $oldHost
...
$strayExe = "$PSScriptRoot\target\release\browser-mcp-backend.exe"
